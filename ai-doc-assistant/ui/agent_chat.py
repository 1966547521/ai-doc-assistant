"""Agent Chat UI — true streaming via st.write_stream + typing animation.
...
| "生成报告" | 调用报告生成 |
    ...
    if query := st.chat_input("用自然语言告诉我你想做什么..."):
        ...
        with st.chat_message("assistant"):
            # Separate containers: tool_status for badges, write_stream for text
            ...
            def _token_gen():
                ...
                for event in stream_agent(query, chat_history, session_id=sid):
                    ...
                    if etype == "tool_start":
                        ...
                        time.sleep(0.008)  # Smooth typing effect
            ...
            # --- TRUE STREAMING via st.write_stream ---
